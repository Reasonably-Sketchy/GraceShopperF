...
const SingleProduct = ({
  ...
  useEffect(async () => {
    const productReviews = await fetchReviews(
      productId,
      numLoadingEvents,
      setNumLoadingEvents
    );
  ...
  const thisProduct = allProducts.find((product) => {
  ...
  const handleAddToCart = async () => {
    ...
    if (token) {
      ...
      const newOrderProduct = await addProductToOrder(
        databaseCart.id,
        body,
        token,
        numLoadingEvents,
    ...
    <main id="single-product">
      ...
      <div className="single-product-body">
        ...
        <section className="single-product-description">
          ...
          <h3>
            <span className="gold-text">{thisProduct.price}</span> USD
          </h3>
          ...
          <div className="action-container">
            ...
            <div className="action-buttons">
              ...
              {userData.isAdmin ? (
                ...
                >
                  Edit Product{" "}
                  {editExpand ? <KeyboardArrowDown /> : <KeyboardArrowRight />}
                </Button>
              ) : (
                ""
              )}
          ...
          <div className="resp-message">
            {respMessage ? (
              <div className="respMessage">{respMessage}</div>
            ) : (
              ""
            )}
          </div>
